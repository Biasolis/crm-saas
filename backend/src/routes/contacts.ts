...
export async function contactRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request, reply) => {
    ...
    try {
      ...
      if (search) {
        queryText += ` AND (c.name ILIKE $2 OR c.email ILIKE $2)`;
  ...
  // --- GET /api/contacts/:id/summary (Visão 360º - NOVO) ---
    ...
    try {
      ...
      const query = `
        INSERT INTO contacts (
          tenant_id, name, email, phone, mobile, document_number, company_name, company_id,
      ...
      const values = [
        tenantId, data.name, data.email, data.phone, data.mobile, data.document_number, data.company_name, companyId,
  ...
  // --- POST /api/contacts/import (Importação CSV) ---
  app.post('/import', async (request, reply) => {
    ...
    try {
      ...
      for (const contact of contacts) {
        ...
        await client.query(`
          INSERT INTO contacts (tenant_id, name, email, phone, mobile, city, state)
          VALUES ($1, $2, $3, $4, $5, $6, $7)
        `, [
          tenantId, contact.name, contact.email || null, contact.phone || null,
